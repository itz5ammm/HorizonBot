  ...
  execute: async (client, message, args) => {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.reply(
        "Sorry, you don't have permissions to use this! please have MANAGE_MESSAGES perm"
      );
    ...
    if (validUnlocks.includes(time)) {
      ...
      message.channel
        ...
        .then(() => {
          message.channel
            .sendMessage(
              `Channel locked down for ${ms(ms(time), { long: true })}`
            )
            .then(() => {
              client.lockit[message.channel.id] = setTimeout(() => {
                message.channel
                  ...
                  .then(message.channel.sendMessage("Lockdown lifted."))
                  .catch(console.error);
                delete client.lockit[message.channel.id];
              }, ms(time));
            })
